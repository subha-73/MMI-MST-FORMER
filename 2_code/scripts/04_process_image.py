...
# This subtracts the mean and divides by std dev of the ImageNet dataset
...
def load_and_process_image(filename):
    ...
    """
    # 1. Construct Path (Handle potential extension mismatches)
    path = os.path.join(ROI_DIR, str(filename))
    
    # Simple check for extension issues (e.g. CSV says .jpg but file is .png)
...
def create_patient_tensors(csv_filename, output_prefix):
    ...
    for pid in tqdm(unique_ids, desc="Building Tensors"):
        ...
        # (If patient has 12 visits, take first 10. If 3, we will pad later.)
        ...
        if pad_len > 0:
            ...
            # This tells the Transformer "Ignore the zeros at the end!"
    ...
    if len(all_patient_imgs) > 0:
        ...
        print(f"  ✓ Missing Images Handled: {missing_files}/{total_files}")
    else:
...
def main():
    ...
    print(f"{'='*70}")
    print("Next Step: Module 2 (Feature Encoders)")
