...
class SpatialEncoder(nn.Module):
    def __init__(self, output_dim=512):
        ...
        # Extract features before global pooling to keep the 7x7 grid tokens
        self.features = nn.Sequential(*list(backbone.children())[:-2])
        self.projector = nn.Conv2d(512, output_dim, kernel_size=1) 

    def forward(self, x):
        # x: [Batch, 3, 224, 224]
        feat_map = self.features(x)       # [Batch, 512, 7, 7]
        return self.projector(feat_map)   # [Batch, output_dim, 7, 7]
'''
#spatial_encoder.py
import torch
import torch.nn as nn
