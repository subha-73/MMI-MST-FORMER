...
def evaluate_best_model():
    print(f"{'='*60}")
    print(f"EVALUATION REPORT: BEST SINGLE SCALE MODEL")
    print(f"{'='*60}")
    ...
    # Note: If you didn't set a manual seed during training, this split 
    # might differ slightly. Ideally, we evaluate on the whole set or a fixed split.
    train_size = int(0.8 * len(dataset))
    val_size = len(dataset) - train_size
    _, val_data = random_split(dataset, [train_size, val_size])
    
    val_loader = DataLoader(val_data, batch_size=BATCH_SIZE, shuffle=False)
    print(f"   ✓ Evaluation Samples: {len(val_data)}")
    ...
    print(f"   ✓ Best Epoch from Training: {checkpoint['epoch']+1}")
    print(f"   ✓ Saved F1 Score: {checkpoint['f1_score']:.4f}")
