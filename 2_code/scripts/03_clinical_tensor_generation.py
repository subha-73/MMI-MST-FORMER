...
def create_clinical_tensors(csv_filename, output_prefix):
    ...
    into padded sequences, and saves the final tensors.
    """
    ...
    all_patient_clin = []
    # patient_labels = [] <--- REMOVED: Static label is not the regression target
    ...
    for pid in tqdm(unique_ids, desc="Building Clinical Tensors"):
        ...
        # label = visits['Progression_Flag'].iloc[0] <--- REMOVED
        ...
        # patient_labels.append(label) <--- REMOVED

    if len(all_patient_clin) == 0: # <--- CRITICAL FIX: Corrected check for zero data
    ...
    # out_label_path = os.path.join(PROCESSED_DIR, f"{output_prefix}_labels.pt") <--- REMOVED

    torch.save(big_tensor_clin, out_clin_path)
    # torch.save(big_tensor_labels, out_label_path) <--- REMOVED

    print(f"  ✓ Saved Clinical Tensor: {out_clin_path}")
    print(f"    Shape: {big_tensor_clin.shape}")
    
    # Optional: Match PIDs to ensure sequence integrity
    pids_from_image_script = os.path.join(PROCESSED_DIR, f"{output_prefix}_pids.txt")
    if os.path.exists(pids_from_image_script):
        # The PIDs here should match those saved by the image script
        # This is a passive check, the DataLoader will use the PIDs from the image script
        pass
